...
每次回车后打印该臂末端 xyz，可据此确认点位真的变了。驱动在控制器掉线/下电/急停时会把最后一帧缓存反馈重打时间戳继续发布（接收 age 很小、时间戳照常推进，但数值逐位不变）；记录器据此做两层提醒：活动臂关节反馈逐位不变超过 2 秒会持续警告；新点与上一点完全相同会标记"此点无效"，几乎相同（<0.002 rad）也会提示。提醒不阻止保存。若看到警告，先活动臂确认 xyz 跟随变化，再继续记录。
